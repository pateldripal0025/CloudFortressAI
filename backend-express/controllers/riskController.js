...
exports.getRisks = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      console.log('[Risks Fallback] MongoDB offline. Querying local risks...');
      let localRisks = fileStorage.getRisks().filter(r => r.tenantId === tenantId && r.environment === environment);

      // Seed initial local risks if empty
      if (localRisks.length === 0 && !search && !severity && !provider) {
        console.log(`[RISK SEED FALLBACK] Initializing local database risks for tenant: ${tenantId}`);
        for (const seed of seedRisksList) {
          await fileStorage.createRisk({
            ...seed,
            environment,
            tenantId
          });
        }
        localRisks = fileStorage.getRisks().filter(r => r.tenantId === tenantId && r.environment === environment);
      }
    ...
    // Normal MongoDB flow
    const query = { environment, tenantId };
    ...
    if (total === 0 && !search && !severity && !provider) {
      console.log(`[RISK SEED] Initializing Advanced Telemetry Matrix for tenant: ${tenantId}`);
      const initialRisks = seedRisksList.map(seed => ({
        ...seed,
        environment,
        tenantId
      }));
...
exports.getRiskById = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      const risk = fileStorage.getRisks().find(r => r._id === req.params.id && r.tenantId === tenantId);
      if (!risk) {
...
exports.resolveRisk = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      ...
      const risk = risks.find(r => r._id === req.params.id && r.tenantId === tenantId);
      if (!risk) {
        return res.status(404).json({ success: false, error: 'Risk not found' });
      }
...
exports.exportRisks = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      risks = fileStorage.getRisks().filter(r => r.tenantId === tenantId);
    } else {
      risks = await Risk.find({ tenantId });
    }
...
exports.exportRiskById = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      risk = fileStorage.getRisks().find(r => r._id === req.params.id && r.tenantId === tenantId);
    } else {
      risk = await Risk.findOne({ _id: req.params.id, tenantId });
    }
    ...
    const fields = ['_id', 'title', 'resource', 'provider', 'severity', 'aiScore', 'recommendation', 'description', 'impact', 'status', 'environment', 'createdAt'];
    const json2csvParser = new Parser({ fields });
...
exports.createRisk = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      const risk = await fileStorage.createRisk(riskData);
...
exports.deleteRisk = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      const risks = fileStorage.getRisks();
      const filtered = risks.filter(r => !(r._id === req.params.id && r.tenantId === tenantId));
      
      if (risks.length === filtered.length) {
        return res.status(404).json({ success: false, error: 'Risk not found' });
      }

      fileStorage.saveRisks(filtered);
      return res.status(204).json({ success: true, data: null });
    }

    const risk = await Risk.findOneAndDelete({ _id: req.params.id, tenantId });
