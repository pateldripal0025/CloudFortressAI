...
import asyncio
from app.scanners.azure_resource_discovery import AzureResourceDiscovery
...
class AzureScanner:
    def __init__(self, user_id: str, scan_id: str):
        ...
        self.discovery = AzureResourceDiscovery()
        self.detector = AzureMisconfigDetector()

    async def run(self):
        logger.info("starting_azure_scan_orchestration", scan_id=self.scan_id, user_id=self.user_id)
        
        try:
            # 1. Discover Resources (offload sync calls to thread)
            ...
            logger.info("completed_azure_scan_orchestration", scan_id=self.scan_id, risks_found=risks_stored)
            return risks_stored

        except Exception as e:
            logger.error("azure_scan_orchestration_failed", scan_id=self.scan_id, error=str(e))
            await scan_repo.update(
                scan_id=str(self.scan_id),
                update_data={
                    "status": "failed",
